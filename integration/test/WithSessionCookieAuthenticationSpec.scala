...
class WithSessionCookieAuthenticationSpec extends PlaySpec with GuiceOneAppPerSuite with AuthenticationHelper {
  ...
  "WithAuthentication controller" should {
    ...
    "update the session on every request" in {
      ...
      val result1 = route(
        app,
        FakeRequest(controllers.routes.WithSessionCookieAuthentication.authenticated).withCookies(sessionCookie1.get)
      ).get
      ...
      val result2 = route(
        app,
        FakeRequest(controllers.routes.WithSessionCookieAuthentication.authenticated).withCookies(sessionCookie2.get)
      ).get
