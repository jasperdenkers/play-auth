...
class UserAuthenticator @Inject() (val configuration: Configuration, val secretConfiguration: SecretConfiguration)
    extends SessionCookieAuthenticator[User, Session]
    with Results {
  ...
  def deserializeSession(map: Map[String, String]) = for {
    ...
    expires <- map.get("expires").flatMap { epochSecondString =>
      Try(epochSecondString.toLong).toOption.map(Instant.ofEpochSecond)
    }
    requestCount <- map.get("requestCount").flatMap { requestCountString =>
      Try(requestCountString.toInt).toOption
    }
  ...
  override def updateSession(session: Session): Future[Session] =
    Future.successful(session.copy(requestCount = session.requestCount + 1))
